...
pub async fn insert_brain_dump(pool: &PgPool, brain_dump: &BrainDump) -> Result<(), BrainDumpError> {
    sqlx::query!(
        r#"
...
pub async fn get_brain_dumps(pool: &PgPool, query: &BrainDumpQuery) -> Result<(Vec<BrainDump>, u64), BrainDumpError> {
...
pub async fn delete_brain_dumps(pool: &PgPool, user_id: uuid::Uuid, dump_ids: &[uuid::Uuid]) -> Result<(), BrainDumpError> {
    sqlx::query!(
        r#"
        UPDATE brain_dumps
        SET deleted_at = NOW()
...
/// Includes already deleted brain dumps
pub async fn user_owns_brain_dumps(pool: &PgPool, user_id: uuid::Uuid, dump_ids: &[uuid::Uuid]) -> Result<bool, BrainDumpError> {
